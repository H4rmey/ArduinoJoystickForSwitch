...
Controller. Howepb_debounce = (pb_debounce << 8) + PINB;ver, by default most of the descriptors are there, with the
...
void GetNextReport(USB_JoystickReport_Input_t* const ReportData) 
{
	...
	//This code was partly written by H4rmey the rest is made by the creater of this: https://github.com/wchill/SwitchInputEmulator repository
	uint16_t bufD = 0x00;
	uint8_t  bufB = 0x00;
	...
	//Using the debounced values
	bufD   	= (~PIND_DEBOUNCED & 0xFF) << (~PINB_DEBOUNCED & 0x08 ? 8 : 0);
	bufB 	= (~PINB_DEBOUNCED & 0xFF);

	//hadling the normal inputs (A, B, X and Y)
	for (int i = 0; i < 4; i++) 
	{
		if (bufD & (1 << i))
	...
	//code for registering inputs for Lz, Rz, L and R (not in order)
	if (bufD & 0x10)
		ReportData->Button |= 0x10;
	if (bufD & 0x80)
		ReportData->Button |= 0x80;
	
	if (bufB & 0x40)
		ReportData->Button |= 0x20;
	if (bufB & 0x02)
		ReportData->Button |= 0x40;
	...
	switch(bufB & 0x3C) {
		...
		case 0x08: // Right
			ReportData->HAT = 0x02;
			break;
		case 0x18: // Bottom-Right
			ReportData->HAT = 0x03;
			break;
		case 0x10: // Bottom
			ReportData->HAT = 0x04;
			break;
		case 0x14: // Bottom-Left
			ReportData->HAT = 0x05;
			break;
		case 0x04: // Left
			ReportData->HAT = 0x06;
